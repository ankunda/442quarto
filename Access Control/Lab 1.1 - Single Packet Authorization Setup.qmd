...
If **Lab: Single Packet Authorization Setup** (this document) is completed within 15 minutes until the end of class, do not begin **Lab: Single Packet Authorization Execution** (part 2). Instead, wait until the next class period to complete part 2.
:::
...
1. On the client, copy the keys you generated to the server using the `scp` command (secure copy). Note that we are copying the keys to a file called `copied-file.conf` at `/home/username` on the server. This is just to migrate the contents from one machine to another. You'll then take some of the contents of this file and move them elsewhere on the server. Additionally, be sure to replace the word `username` with the name of the user of the server.

    ```sh 
    scp -P $SSH_PORT ~/.fwknoprc username@$SERVER_IP:~/copied-file.conf
    ```
...
2. On the server, open the file at `/etc/fwknop/access.conf` and find the section that has the following information:
    ...
    ii. Replace both instances of `__CHANGE_ME__` by copying and pasting the keys from the ~/copied-file.conf file you moved onto the server. 

    iii. Add a line after SOURCE that is labeled OPEN_PORTS with a value of tcp/22. 

    iv. Confirm the file looks similar to the following. The values for SOURCE, KEY_BASE64, and HMAC_KEY_BASE64 will be different from those shown.
...
3. On the server, remove the file you placed at the home directory. That is, remove `~/copied-file.conf`.
...
4. Complete the following on the server to enable fwknop 
    ...
    iv. Open the file `/etc/fwknop/fwknopd.conf` with nano or vim. This is the fwknop daemon config.

    v. Find the line with `PCAP_INTF`. If it exists, uncomment it. By default it shows `eth0`, which can stay as that for Linux machines. On newer distributions (Ubuntu 24.04+ and recent Mint releases), this line has been removed from the default config and fwknopd uses NFQ (NetFilter Queue) instead. If you do not see a `PCAP_INTF` line in the file, you can skip ahead — the default NFQ configuration will work without any edits.
